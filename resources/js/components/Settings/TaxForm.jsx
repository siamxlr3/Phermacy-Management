...
const TaxForm = ({ initialData, onClose }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({ name: initialData.name, rate: initialData.rate, status: initialData.status });
    } else {
          ...
          <input
            ...
            className={`w-full px-3.5 py-2.5 text-sm bg-white border rounded-xl transition-all focus:outline-none focus:ring-2 ${
              errors.name
                ...
                : 'border-slate-200 focus:ring-emerald-500/20 focus:border-emerald-400 hover:border-slate-300'
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full pl-3.5 pr-10 py-2.5 text-sm bg-white border rounded-xl transition-all focus:outline-none focus:ring-2 ${
                errors.rate
                  ? 'border-red-300 focus:ring-red-500/20 focus:border-red-400'
                  : 'border-slate-200 focus:ring-emerald-500/20 focus:border-emerald-400 hover:border-slate-300'
              } placeholder:text-slate-300`}
            />
            <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-medium pointer-events-none">%</span>
          </div>
          {errors.rate && <p className="text-xs text-red-500 mt-1.5">⚠ {errors.rate}</p>}
        </div>
