...
import { useAddCategoryMutation, useUpdateCategoryMutation } from '../../store/api/medicineApi';
import { X, Loader2, Folder } from 'lucide-react';
import toast from 'react-hot-toast';

const CategoryForm = ({ initialData, onClose }) => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {initialData ? 'Modify the category info' : 'Add a new group for medicines'}
            </p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <div>
          ...
          <input
            ...
            className={`w-full px-3.5 py-2.5 text-sm bg-white border rounded-xl transition-all focus:outline-none focus:ring-2 ${
              errors.name ? 'border-red-300 focus:ring-red-500/20 focus:border-red-400' : 'border-slate-200 focus:ring-emerald-500/20 focus:border-emerald-400 hover:border-slate-300'
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className={`w-full px-3.5 py-2.5 text-sm bg-white border rounded-xl transition-all focus:outline-none focus:ring-2 border-slate-200 focus:ring-emerald-500/20 focus:border-emerald-400 hover:border-slate-300 placeholder:text-slate-300 resize-none`}
          />
        </div>
