...
import { useGetManufacturersQuery, useDeleteManufacturerMutation } from '../../store/api/medicineApi';
import { Search, Plus, Pencil, Trash2, Factory, ChevronLeft, ChevronRight } from 'lucide-react';
...
const EmptyState = ({ onAdd }) => (
  <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
    ...
    <h3 className="text-base font-semibold text-slate-700 mb-1">No manufacturers found</h3>
    <p className="text-sm text-slate-400 mb-5 max-w-xs">
      Add the pharmaceutical companies and manufacturers you source medicines from.
    </p>
    <button
      onClick={onAdd}
    ...
    >
      <Plus size={15} /> Add First Manufacturer
    </button>
  </div>
);
...
const ManufacturerTable = () => {
  ...
  const { data, isLoading, isFetching } = useGetManufacturersQuery({ page, perPage, search: debouncedSearch });
  const [deleteManufacturer] = useDeleteManufacturerMutation();

  const handleDelete = async (id) => {
    if (confirm('Delete this manufacturer? This action cannot be undone.')) {
      try {
  ...
  const handleEdit = (manuf) => { setEditingManufacturer(manuf); setIsFormOpen(true); };
  const handleAdd = () => { setEditingManufacturer(null); setIsFormOpen(true); };
    ...
    <div className="flex gap-6 h-full min-h-0">
      <div className="flex-1 min-w-0 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col min-h-0">
        <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="text-base font-semibold text-slate-900">Manufacturers</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              {isLoadingState ? '...' : `${meta.total || manufacturers.length} pharmaceutical partners`}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar">
          {manufacturers.length === 0 && !isLoadingState ? (
              ...
              <tbody className="divide-y divide-slate-50">
                {isLoadingState
                  ...
                  : manufacturers.map((manuf) => (
                    <tr key={manuf.id} className="group hover:bg-slate-50/50 transition-colors duration-100">
                      <td className="px-6 py-4 w-1/4">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
                            <Factory size={14} />
                          </div>
                          <span className="text-sm font-semibold text-slate-800">{manuf.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 w-2/5">
                        <span className="text-sm text-slate-500 line-clamp-1">{manuf.description || '—'}</span>
                      </td>
                      <td className="px-6 py-4 text-center w-1/6">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold tracking-wide ${
                          ...
                          <span className={`w-1.5 h-1.5 rounded-full ${manuf.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                          {manuf.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right w-1/6">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                          <button onClick={() => handleEdit(manuf)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                            <Pencil size={14} />
                          </button>
                          <button onClick={() => handleDelete(manuf.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
      ...
      <AnimatePresence>
        {isFormOpen && (
          ...
          >
            <ManufacturerForm initialData={editingManufacturer} onClose={() => setIsFormOpen(false)} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
